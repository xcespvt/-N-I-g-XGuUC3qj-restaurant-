    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Select how much extra time you need to prepare this order. The
...
export const CountdownTimer = ({ order }: { order: Order }) => {
  ...
  useEffect(() => {
    const calculateRemainingTime = () => {
      ...
      const newProgress =
        totalPrepSeconds > 0 ? (remainingSeconds / totalPrepSeconds) * 100 : 0;
      ...
      <div className="border rounded-lg p-3 space-y-3 bg-background">
        <div className="flex items-center justify-between text-sm">
          ...
          <div
            className={cn(
              "font-bold text-lg",
              isTimeUp ? "text-red-500" : "text-primary"
            )}
          >
            {isTimeUp
              ? "Time's up!"
