...
export function TakeawayDineinOrderDetails({ order }: OrderDetailViewProps) {
  const { updateOrderStatus, selectedBranch, orders } = useAppStore();
    ...
    `/api/orders/${selectedBranch}/${order.id}/prep-time`,
    {
      onSuccess: (response: any) => {
        if (response.success && response.data) {
          // Update the specific order in the global state
          const updatedOrder = { 
            ...order, 
          ...
          // Refetch instantly to show new timings across the app
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 overflow-y-auto pb-24">
      ...
      <div className="p-4 space-y-[14px]">
        ...
        {(order.type === "Dine-in" ||
          order.customerDetails.address.includes("Table")) && (
          <div className="bg-white rounded-[16px] p-4 border border-[#E5E7EB]">
            <h2 className="text-base font-semibold text-slate-900 mb-4">
              Table Information
            </h2>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-900">
                    {order.customerDetails.address.replace("Table: ", "")}
                  </p>
