...
import { IndianRupee, Plus, Search, X, Minus, ShoppingCart, ArrowRight } from "lucide-react"
...
import type { MenuItem, TakeawayCartItem } from "@/context/AppContext"
...
const MenuItemCard = ({ item }: { item: MenuItem }) => {
    ...
    const quantityInCart = useMemo(() => {
        return takeawayCart.filter(cartItem => cartItem.id === item.id)
        ...
        <>
            <div className="flex gap-4 border-b pb-6">
                <div className="flex-1 flex flex-col justify-between">
                    <div>
                        <div className={cn("inline-flex items-center gap-1.5 px-1.5 py-0.5 rounded-sm text-xs font-semibold", item.dietaryType === 'Veg' ? 'border border-green-600 text-green-600' : 'border border-red-600 text-red-600')}>
                            <div className={cn("h-2 w-2 rounded-full border", item.dietaryType === 'Veg' ? 'bg-green-600 border-green-700' : 'bg-red-600 border-red-700')}></div>
                            {item.dietaryType}
                        </div>
                        <h3 className="font-bold text-base mt-2">{item.name}</h3>
                         <p className="font-semibold text-sm flex items-center mt-1">
                            <IndianRupee className="h-3.5 w-3.5" />{item.price.toFixed(0)}
                        </p>
                    </div>
                    <div className="mt-4">
                       {quantityInCart > 0 ? (
                            <Button variant="outline" className="h-9 w-32" onClick={handleButtonClick}>
                                <span className="w-full text-center">{quantityInCart} in cart</span>
                           </Button>
                       ) : (
...
const ItemSelectionDialog = ({ isOpen, onOpenChange, item }: { isOpen: boolean, onOpenChange: (open: boolean) => void, item: MenuItem }) => {
    ...
    const portionInCart = useMemo(() => {
        return takeawayCart.find(cartItem => cartItem.id === item.id && cartItem.portion === selectedPortionName);
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            const cartItem = takeawayCart.find(ci => ci.id === item.id && ci.portion === firstPortionName);
            setQuantity(cartItem?.quantity || 1);
        }
    }, [isOpen, item, takeawayCart]);
    
     useEffect(() => {
        const cartItem = takeawayCart.find(ci => ci.id === item.id && ci.portion === selectedPortionName);
        setQuantity(cartItem?.quantity || 1);
    }, [selectedPortionName, item.id, takeawayCart]);


    const selectedPortion = useMemo(() => {
        return item.portionOptions?.find(p => p.name === selectedPortionName) || { name: 'Full', price: item.price };
            ...
            <DialogContent className="sm:max-w-md p-0">
                ...
                <div className="px-6 space-y-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {item.portionOptions.map(portion => (
                                    ...
                                    >
                                        {portion.name} - <IndianRupee className="h-3.5 w-3.5 mx-1" />{portion.price}
                                    </Button>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <Button variant="outline" size="icon" className="h-9 w-9" onClick={() => setQuantity(q => Math.max(1, q-1))} disabled={!portionInCart && quantity === 1}>
                ...
                <DialogFooter className="bg-muted p-4 mt-6 flex-col-reverse sm:flex-row sm:justify-between sm:items-center gap-4 border-t">
                     <p className="font-bold text-xl flex items-center">
                        Total: <IndianRupee className="h-5 w-5 mx-1" />{(selectedPortion.price * quantity).toFixed(2)}
                     </p>
                    <div className="flex gap-2 w-full sm:w-auto">
                       {portionInCart ? (
                            <Button variant="ghost" className="text-destructive flex-1 sm:flex-initial" onClick={handleRemoveFromCart}>Remove</Button>
        ...
        <Card className="lg:sticky lg:top-6">
            ...
            <CardContent>
                    ...
                    <div className="space-y-4">
                                ...
                                <div key={item.cartItemId} className="flex items-start justify-between gap-2">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            {item.portion !== 'Full' && `(${item.portion})`} <span className="flex items-center"><IndianRupee className="h-3 w-3" />{item.price.toFixed(2)}</span>
                                        </p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => decrementTakeawayCartItem(item.cartItemId)}>
                                            <Minus className="h-4 w-4"/>
                                        </Button>
                                        <span className="font-bold text-sm w-6 text-center">{item.quantity}</span>
                                        <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => incrementTakeawayCartItem(item.cartItemId)}>
                                            <Plus className="h-4 w-4"/>
                                        </Button>
                                    </div>
                                    <p className="font-semibold text-sm w-16 text-right flex items-center justify-end"><IndianRupee className="h-3.5 w-3.5" />{(item.price * item.quantity).toFixed(2)}</p>
                                </div>
                            ))}
                        </div>
                        <Separator />
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                                ...
                                <p className="font-medium flex items-center"><IndianRupee className="h-3.5 w-3.5" />{cartTotal.toFixed(2)}</p>
                            </div>
                             <div className="flex justify-between">
                                <p className="text-muted-foreground">Taxes (18% GST)</p>
                                <p className="font-medium flex items-center"><IndianRupee className="h-3.5 w-3.5" />{(cartTotal * 0.18).toFixed(2)}</p>
                            </div>
                        </div>
                        <Separator />
                        <div className="flex justify-between items-center text-lg font-bold">
                            <p>Total</p>
                             <p className="flex items-center"><IndianRupee className="h-5 w-5" />{(cartTotal * 1.18).toFixed(2)}</p>
                        </div>
                    </div>
                ) : (
             ...
             <CardFooter className="flex-col gap-2 border-t pt-4">
                 ...
                 <Button variant="ghost" className="w-full text-destructive" onClick={clearTakeawayCart} disabled={takeawayCart.length === 0}>
                     Clear Order
                 </Button>
            </CardFooter>
        </Card>
